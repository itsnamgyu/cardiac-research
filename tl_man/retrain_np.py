...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
    ...
    resized_input_values = np.load(image_data)

    """
    // save all as np
    head, tail1 = os.path.split(image_path)
    _, tail2 = os.path.split(head)
    filename, _ = os.path.splitext(tail1)
    np_path = "./np/" + tail2 + "/" + filename + ".npy"
    print(image_path)
    print(np_path)
    np.save(np_path, resized_input_values)
    
    np.set_printoptions(precision=4)
    print("Resized Input Values:", type(resized_input_values))
    print(resized_input_values.shape)
    print(resized_input_values)
    exit();
    """
    ...
    # print(bottleneck_tensor.numpy()) # only possible w/ eager execution
